...
> 历史诊断索引：下列模型语义成绩与当时状态保留原样，不再作为发布门槛或当前待修清单。用户已明确由软件保证准确传达与真实执行，不以模型绝对服从为目标；最新软件修复及交付状态统一见[3.0 故障表](3.0-bug-board.md)。文中的“不能扩大”“完整旅程待验”等历史结论须按主计划顶部的最新范围纠正理解。
