...
### 净眼二审（2026-09-18，裁决：可合并）

三处修复逐项成立：messageIsStreaming 值链闭环（活流占位 true/overlay
强制/终局清 false/DB 重载恒 false/resume 新开占位）；foldable 三态对上；
liveStatuses 无重复遗漏、纯飞行回合无行可标可接受。P3 观察项：交互类
工具活流飞行也计入 liveStatuses（标签语义反而更准确）。正则分歧以
python repr 字节级复核 + CI 实证关账（净眼误读 raw string 收尾边界）。

### 守纲终裁（2026-09-18，裁决：准予合并）

六问全过：七文件零夹带/纯展示层无出口/折叠单一定义无并存/一字段一列表
两正则无过度工程/ChatViewModel 零 diff/一审四项处置逐一对上代码。附加：
无外溢面；"小字滚动+工具折叠"两词兑现；常驻开关/思考流/逐字全览挂账下批。

## 终态

三闸全过（CI 绿 de10284 / 净眼二审可合并 / 守纲准予），PR #28 合并入 next。
交付：活流工具行直播（不折叠+滚动尾巴）+ 工作行进行中标签修复。
- CI：704dc2e 红（可空调用，3201d2d 修复）；**终版 de10284 绿**。
