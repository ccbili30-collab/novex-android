...
- CI：一审 ee844fe 红（漏 import，a3f474c 修复）；二审 83646e3 红
  （assertSame 两次构造，167f46e 修复）；**终版 f20ca27 绿**。

## 终态

三闸全过（CI 绿 / 净眼二审可合并 / 守纲准予合并），PR #26 合并入 next。
交付物：恢复指令 one-shot 化 + 选项回应语境标记 + 测试墙 11 用例。
挂账去处：P3-5（choiceRepairAttempted 仅日志）、P3-8（VM 接线层无纯函数
可测）记本任务书；P3-a~d 见净眼二审段。
