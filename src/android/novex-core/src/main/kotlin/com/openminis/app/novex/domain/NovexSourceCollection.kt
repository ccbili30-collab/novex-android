...
data class NovexLearningNote(
    ...
    val sourceRevisions: Map<NovexResourceRef, String> = emptyMap(),
) {
    init {
        ...
        require(sourceRevisions.keys.all { it in sourceDocumentRefs } && sourceRevisions.values.all { it.matches(Regex("[0-9a-f]{64}")) }) {
            "笔记来源修订必须属于声明的文档"
        }
