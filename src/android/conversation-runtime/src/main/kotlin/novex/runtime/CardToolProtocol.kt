...
object CardToolProtocol {
    fun definitions(policy:CardToolPolicy):List<ToolDefinition> {
        ...
        return CardEditingTools.definitions()+listOf(
            ...
            tool("move_module","将目标卡中的一个模块移到另一个模块之前，或移到末尾。保留模块内容、编号、标签与携带规则；先读取最新结构；保存结果的 next_draft_version 可直接用于下一次修改。",mapOf("module_id" to "要移动的模块编号","before_id" to "同父级定位模块编号，空字符串表示同级末尾；改变父级用 place_module")),
            tool("move_content_block","在同一模块内调整文字块或图片块顺序，不重写正文或图片。图片块的结构类型为 image_caption，但移动的是整块图片展示，不仅是图注。先读取最新结构；保存结果的 next_draft_version 可直接用于下一次操作。",mapOf("module_id" to "所属模块编号","block_id" to "要移动的文字或图片块编号","before_id" to "同模块定位块编号，空字符串表示末尾")),
            ...
            tool("write_module_text","新建或完整替换一个文字块并保存。新建时 block_id 传空字符串（缺省或误填 new 等占位词也按新建处理）；该操作不创建子模块。保存结果返回新建块编号 created_block_ids 和 next_draft_version——下一步直接引用它们继续操作，无需重读结构。不要把读到的一页当成全文覆盖；局部修改使用 replace_text_range。失败不得宣称完成。",mapOf("module_id" to "目标模块编号","block_id" to "已有文字块编号；新建文字块时为空字符串","name" to "模块名称","text" to "完整的新文字块正文")),
    ...
    fun parse(chatId:String,call:PendingTool):CardToolRequest {
        ...
        val edit=when(call.name) {
            ...
            "replace_text_range"->{
                ...
                fun position(key:String):Long=when(val raw=value.get(key)){is Long->raw;is Int->raw.toLong();is Double->raw.toLong();is String->raw.trim().toLongOrNull()?:error("$key 必须是整数");else->error("$key 必须是整数")}
