...
class NovexConversationDraftPersistenceTest {
    ...
    fun `explicitly addressed empty role remains readable as a creation target without becoming a mounted card`() = runBlocking {
        ...
        try {
            ...
            assertTrue(read.subjects.isEmpty())
            assertEquals(3, read.draftTargets.size)
            assertTrue(read.draftTargets.any { it.subject == role })
    ...
    fun `migration preserves shared cards and failed trio creation rolls back every new row`() = runBlocking {
        ...
        fun openDatabase() = Room.databaseBuilder(context, AppDatabase::class.java, path)
            .addMigrations(AppDatabase.MIGRATION_25_26, AppDatabase.MIGRATION_26_27, AppDatabase.MIGRATION_27_28, AppDatabase.MIGRATION_28_29, AppDatabase.MIGRATION_29_30, AppDatabase.MIGRATION_30_31, AppDatabase.MIGRATION_31_32, AppDatabase.MIGRATION_32_33, AppDatabase.MIGRATION_33_34, AppDatabase.MIGRATION_34_35, AppDatabase.MIGRATION_35_36, AppDatabase.MIGRATION_36_37, AppDatabase.MIGRATION_37_38).allowMainThreadQueries().build()
        var database = openDatabase()
        val media = File(files.root, "media")
        try {
            var workspace = NovexWorkspaceFactory.create(database, media)
