...
class ImportedWorldEntryTest {
    @Test fun importedSingleModuleCanStartAt128KWithoutClaimingFullRead()=runBlocking {
        ...
        for(budget in listOf(100000,64000,8192)) {
            ...
            assertTrue("candidate/fragments mismatch: candidates=${material.map { it.sourceId to it.partial }} fragments=${composed.fragments.map { it.sourceId to it.partial }}", material.all { candidate ->
                val fragment = composed.fragments.firstOrNull { it.sourceId == candidate.sourceId }
                if (candidate.partial) fragment == null || fragment.partial
                else fragment != null && !fragment.partial && fragment.text == candidate.content.trim()
            })
