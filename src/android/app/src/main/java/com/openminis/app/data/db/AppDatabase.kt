...
@Database(
    entities = [
        ...
        NovexCharacterRevisionEntity::class,
        NovexWorkGroupEntity::class,
        NovexWorkGroupMemberEntity::class,
    ...
    companion object {
        val MIGRATION_38_39 = object : Migration(38, 39) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val exists = db.query("PRAGMA table_info(sessions)").use { cursor ->
                    var found = false
                    while (cursor.moveToNext()) if (cursor.getString(cursor.getColumnIndexOrThrow("name")) == "text_style_prompt") found = true
                    found
                }
                if (!exists) db.execSQL("ALTER TABLE sessions ADD COLUMN text_style_prompt TEXT")
            }
        }
        ...
        val MIGRATION_37_38 = object : Migration(37, 38) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                if ("runtime_dice_enabled" !in columns) {
                    db.execSQL("ALTER TABLE sessions ADD COLUMN runtime_dice_enabled INTEGER NOT NULL DEFAULT 0")
                }
                if ("runtime_ledger_enabled" !in columns) {
                    db.execSQL("ALTER TABLE sessions ADD COLUMN runtime_ledger_enabled INTEGER NOT NULL DEFAULT 0")
                }
            }
        }
        ...
        val MIGRATION_36_37 = object : Migration(36, 37) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val exists = db.query("PRAGMA table_info(sessions)").use { columns ->
                    var found = false
                    while (columns.moveToNext()) if (columns.getString(columns.getColumnIndexOrThrow("name")) == "side_of_session") found = true
                    found
                }
                if (!exists) db.execSQL("ALTER TABLE sessions ADD COLUMN side_of_session TEXT")
            }
        }
        ...
        val MIGRATION_30_31 = object : Migration(30, 31) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_world_revisions (world_id TEXT NOT NULL, sequence INTEGER NOT NULL, saved_at INTEGER NOT NULL, content_json TEXT NOT NULL, PRIMARY KEY(world_id, sequence), FOREIGN KEY(world_id) REFERENCES worlds(id) ON UPDATE NO ACTION ON DELETE CASCADE)")
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_game_revisions (project_id TEXT NOT NULL, sequence INTEGER NOT NULL, saved_at INTEGER NOT NULL, content_json TEXT NOT NULL, PRIMARY KEY(project_id, sequence), FOREIGN KEY(project_id) REFERENCES interactive_fiction_projects(id) ON UPDATE NO ACTION ON DELETE CASCADE)")
            }
        }
        val MIGRATION_29_30 = object : Migration(29, 30) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_work_groups (id TEXT NOT NULL PRIMARY KEY, name TEXT NOT NULL)")
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_work_group_members (group_id TEXT NOT NULL, kind TEXT NOT NULL, target_id TEXT NOT NULL, PRIMARY KEY(group_id, kind, target_id), FOREIGN KEY(group_id) REFERENCES novex_work_groups(id) ON UPDATE NO ACTION ON DELETE CASCADE)")
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_work_group_selection (id INTEGER NOT NULL PRIMARY KEY, selection TEXT NOT NULL)")
            }
        }
        val MIGRATION_28_29 = object : Migration(28, 29) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_character_revisions (version_id TEXT NOT NULL, sequence INTEGER NOT NULL, saved_at INTEGER NOT NULL, content_json TEXT NOT NULL, PRIMARY KEY(version_id, sequence), FOREIGN KEY(version_id) REFERENCES character_versions(id) ON UPDATE NO ACTION ON DELETE CASCADE)")
            }
        }
        @Volatile
        ...
        val MIGRATION_26_27 = object : Migration(26, 27) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_card_references (id TEXT NOT NULL PRIMARY KEY, source_kind TEXT NOT NULL, source_id TEXT NOT NULL, source_module_id TEXT, target_kind TEXT NOT NULL, target_id TEXT NOT NULL, position INTEGER NOT NULL, content_json TEXT NOT NULL)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_novex_card_references_source ON novex_card_references (source_kind, source_id)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_novex_card_references_target ON novex_card_references (target_kind, target_id)")
            }
        }

        val MIGRATION_25_26 = object : Migration(25, 26) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("CREATE TABLE IF NOT EXISTS novex_conversation_drafts (conversation_id TEXT NOT NULL PRIMARY KEY, content_json TEXT NOT NULL)")
            }
        }
