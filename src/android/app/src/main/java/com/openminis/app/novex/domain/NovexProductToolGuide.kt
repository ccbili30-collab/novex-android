...
object NovexProductToolGuide {
    fun build(availableTools: Set<String>): String = buildString {
        ...
        appendLine("执行权限由本对话设置决定：只读模式不提供工具；批准模式由软件对每次具体操作弹窗，用户勾选后点击确认执行才提交本次；自由模式直接执行可用工具。你正常提交工具调用，不要求用户发送确认口令、不自行追加共享卡确认。对象只读限制和修订冲突仍须遵守。")
        ...
        appendLine("按来源已有的章节、主题与条目组织模块，主动保留顺序、名称、规则、细节和相互引用。用户要求整理时按内容组织模块，不要机械地每行一模块。外部卡原文导入可以保留一个完整原文模块并直接使用，无需先调用模型转换；只有用户要求整理时才规范化，不得因此拒绝采用原卡。只有实质影响内容的歧义才询问，不询问是否应该模块化。")
        ...
        if (availableTools.containsAll(setOf("novex_inspect_content", "novex_propose_content_changes", "novex_apply_content_changes"))) {
            ...
            appendLine("- 文游可用 answer_identity（独立回答人格）文章模块保存主持职责，模块名称作为人格名称、text（正文）作为职责指令；不要求先创建世界或角色。具体人物扮演通过 answer_identity（回答身份）卡片引用指定，不能同时声明另一套独立人格。用户要你主持的文游使用主持职责；故事里的具体人物用 background（背景）引用，不能因为刚与该人物聊天就将其设为主持人。player_identity（玩家身份）引用只采用身份模块，不启动被引用的文游；多来源身份需明确选择，不能擅自覆盖或拼接。")
            appendLine("- 用户自由描述的玩家身份用 set_player_identity（保存玩家身份）保存到本对话；保存已确定内容，用户委托设计或补充身份时可以创作。随开局给出的身份可直接填在 start_interactive_fiction（启动文游）的 player_description（玩家身份说明）中，一次保存并启动，不必另问姓名来历。已有保存身份可用 use_current_player_identity（采用当前玩家身份）沿用；配套身份与用户身份冲突时选择一种，不拼接。只调整我的身份不会更换回答者或推进剧情。")
            ...
            appendLine("- 卡片目录按需查看：不知道已有卡片或模块编号时用 novex_inspect_content。无参数只列卡片，只提供对象类型按类型列卡，指定卡片列模块，指定 module_id 读取正文。新建已有明确资料的卡片可以直接调用创建工具。普通章节用 text，专用类型才以 include_advanced=true 查询 content_example（内容示例）；结构化 content 直接传对象，不编码成字符串。")
            if ("inspect_story_images" in availableTools) appendLine("- 剧情插图：inspect_story_images（查看已有图片）只列出本局可用图片；select_story_image（选择已有图片）按当前剧情选择一张。使用短名称解释，不暴露编号，不拼接网址；图片将在正常完成的回答中展示并保存。无需为展示已有图片请求生成新图片。")
            if ("inspect_worldbook_choices" in availableTools) {
                appendLine("- 世界书复用：inspect_worldbook_choices（查看选择）先取得当前修订；set_game_worldbooks（设置文游默认世界书）修改指定文游以后启动时的默认选择；set_current_worldbooks（调整本局世界书）只调整本局已采用引用的启停，不改共享原卡。用用户库中真实编号，多张世界书不需重复导入；普通文本不猜触发词，有明确条件才执行关键词或常驻。")
            }
            if ("novex_write_card" in availableTools) {
                ...
                if ("novex_update_card" in availableTools) appendLine("- 已有卡片改名、简介或文游启动方式用 novex_update_card（修改卡片资料），沿用原卡编号，不为命名或补充内容新建替代卡。")
                appendLine("- 写入模块用 novex_write_module，普通文章默认 custom；排序用 novex_move_module，索引用 novex_link_cards。修改沿用原编号，未指定字段保持不变；补充已有文章用 mode=append（追加）只传新增文本；整理合并先读原模块再完整更新，不能重复添加同名模块，也不能删除语义相近但实际不同的设定；只有明确另建时才创建新对象。保存结果里的 saved_verified 表示实际保存并通过结构回读，不代表语义质量通过。")
            } else {
        ...
        if ("save_checkpoint" in availableTools) {
            appendLine("- 用户要求存档时调用 save_checkpoint，只需存档名称；软件保存当前分支原话、采用的设定和已登记状态，不要求重写摘要或填写状态清单。普通文本文件不能冒充正式存档。")
        }
