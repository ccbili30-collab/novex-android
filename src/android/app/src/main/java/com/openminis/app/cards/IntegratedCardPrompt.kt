...
object IntegratedCardPrompt {
    fun build(style:String,memory:Boolean,tools:Set<String>):NovexSystemPrompt.Prepared {
        ...
        val prompt=buildString {
            ...
            appendLine("创建整卡或一次性补充大批量内容必须用 create_card_bulk / add_module_bulk（嵌套模块树整包校验后原子写入，不要逐个 add_module 搭骨架再填）。微调单模块时：write_module_text（保存文字块）新建文字块的 block_id（文字块编号）传空字符串或直接省略均可，不能猜 new、create 等编号；或用 write_module_markdown（保存模块图文）。完成前读取实际结构，核对分组和子模块是否符合独立编辑需要；不要把正文写入成功当作模块组织已经完成。保留用户指定的原文结构，未经要求不重排现有作品。")
            appendLine("整理纪律：你写进卡的内容是长期资产，会被反复携带，口癖会被放大。用户提供的文本原样入卡——可拆分、加标题、提表格，但一字不改、未经要求不润色；整理是结构化，不是改写。专有名词与称谓沿用原料写法，不自创同义词。")
            appendLine("文中立：必须由你写的文字（标题、表格行、连接句）保持平实——短句、白描、零修辞；不用排比堆砌、“仿佛/宛如”引导的比喻、“不是……而是……”句式，也不写段尾升华。文风以用户原文或用户明确要求为准，不代入你自己的表达习惯。")
