...
interface NovexWorkGroupDao {
    ...
    @Query("UPDATE novex_work_groups SET organization_json = :json WHERE id = :id")
    suspend fun organize(id: String, json: String)
    ...
    @Query("SELECT CASE WHEN :kind = 'WORLD' THEN EXISTS(SELECT 1 FROM worlds WHERE id = :id) WHEN :kind = 'CHARACTER_VERSION' THEN EXISTS(SELECT 1 FROM character_versions WHERE id = :id) WHEN :kind = 'INTERACTIVE_FICTION' THEN EXISTS(SELECT 1 FROM interactive_fiction_projects WHERE id = :id) WHEN :kind = 'CREATIVE_ARTIFACT' THEN EXISTS(SELECT 1 FROM creative_artifacts WHERE id = :id AND trashed_at IS NULL) ELSE 0 END")
