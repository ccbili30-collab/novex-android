...
object NovexCardFileTools {
    ...
    private val body = linkedMapOf(
        ...
        "content" to AgentToolParam("object", "专用模块的结构化内容对象，使用高级目录中的内容示例；直接传对象，不转成字符串。普通文章直接用 text。"),
        "source" to source)
    fun definitions() = listOf(
        ...
        AgentToolDefinition(WRITE_MODULE,
            ...
            required = emptyList()),
        AgentToolDefinition(MOVE_MODULE, "调整一个模块的排列位置，其余正文和模块保持不变。执行权限由对话设置统一决定。",
