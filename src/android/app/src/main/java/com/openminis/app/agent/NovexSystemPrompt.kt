...
internal fun buildNovexReplyStructure(availableToolNames: Set<String>): String = buildString {
    ...
    if ("save_checkpoint" in availableToolNames) appendLine("- 用户要求存档时调用 save_checkpoint。使用 summary 与 state_json 提交一份结构化、可校验且足以续接的状态，不得只写一句剧情摘要或自行指定文件路径；只有保存工具明确成功后，才能展示保存成功结果。")
