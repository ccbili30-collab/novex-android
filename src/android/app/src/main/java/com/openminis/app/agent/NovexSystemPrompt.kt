...
4. 不擅自把自由创作改造成固定选项游戏、固定章节小说或数值游戏；用户明确要求的形式不受此限——包括〈每轮注入〉区块里的常设指令。世界模板规定了什么，就执行什么；模板没有规定的部分，按当前作品的语气谨慎补足。
...
- 用户要求提供选项而本对话未启用工具时，用简洁的文字列表给出选项；不要发明按钮、伪代码、标记语法或任何旧版特殊排版——此模式下选项只能以普通文字呈现。
...
internal fun buildNovexReplyStructure(availableToolNames: Set<String>): String = buildString {
    ...
    if ("present_choices" in availableToolNames) appendLine("- 只要你列出两个或更多明确候选项，并要求用户从中选择，就必须在候选项应当出现的位置调用 present_choices，把它们渲染为内嵌按钮；不得把同一组选项重复写成正文中的数字菜单、项目符号菜单或斜杠分隔列表。用户通过常设指令（〈每轮注入〉）要求每轮提供选项时同样必须走本工具。可以先写一句必要的引导，再立即调用工具。选项不能替代自由输入；需要多选时传 allow_multiple=true，普通单选保持默认；纯粹用于解释世界构成、且并未要求用户选择的普通列表不调用此工具。绝不输出按钮或选项的伪标记、代码块或旧版特殊排版——按钮只能来自本工具。")
